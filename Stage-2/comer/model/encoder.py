...
class _Bottleneck(nn.Module):
    def __init__(self, n_channels: int, growth_rate: int, use_dropout: bool, sd_prob: float):
...
class DenseNet(nn.Module):
    def __init__(
        ...
        self.total_blocks = n_dense_blocks * 3  # Three dense layers
        sd_probs = [stochastic_depth_prob * i / (self.total_blocks - 1) for i in range(self.total_blocks)]

        self.dense1 = self._make_dense(
            n_channels, growth_rate, n_dense_blocks, bottleneck, use_dropout, sd_probs[:n_dense_blocks]
        )
        ...
        self.dense2 = self._make_dense(
            n_channels, growth_rate, n_dense_blocks, bottleneck, use_dropout, sd_probs[n_dense_blocks:2*n_dense_blocks]
        )
        n_channels += n_dense_blocks * growth_rate
        n_out_channels = int(math.floor(n_channels * reduction))
        ...
        self.dense3 = self._make_dense(
            n_channels, growth_rate, n_dense_blocks, bottleneck, use_dropout, sd_probs[2*n_dense_blocks:]
        )
    ...
    @staticmethod
    def _make_dense(n_channels, growth_rate, n_dense_blocks, bottleneck, use_dropout, sd_probs):
        layers = []
        for i in range(int(n_dense_blocks)):
            sd_prob = sd_probs[i]
            if bottleneck:
                layers.append(_Bottleneck(n_channels, growth_rate, use_dropout, sd_prob))
            else:
                layers.append(_SingleLayer(n_channels, growth_rate, use_dropout, sd_prob))
